...
function assemble_system(mdl::model; GRAD::Bool=false)
    ...
    if mdl.nDof == 1
        ...
        if GRAD == true
            ...
            dVdλ = zeros(Float64, mdl.ne^mdl.ndim*((ID_rows*IEN_rows)^2))  
            dVdμ = zeros(Float64, mdl.ne^mdl.ndim*((ID_rows*IEN_rows)^2))
        end
    end
    ...
    for gp in gpiter
        ...
        for e in e_iter
            ...
            if mdl.nDof == 1
                ...
                dKedμ = B*mdl.dcMatdμ*B'*w # element stiffness matrix
                        ...
                        for iDof in iDofs
                            for jDof in jDofs
                                ...
                                if GRAD == true
                                    ...
                                    dJ[inz] = mdl.ID[jDof,mdl.IEN[jNode,e]] # column 
    ...
    if GRAD == true
        dKdλ = sparse(dE,dJ,dVdλ)
        dKdμ = sparse(dE,dJ,dVdμ)
        return K, dKdλ, dKdμ
    else
        return K
    end
...
# Returns:
- `output::AbstractArray` : solution fields of the border nodes in 2D
- `gradList::AbstractArray{Vector{Float64,2,nParameters}}` : solution fields of the border nodes in 2D
- `borderPts2DList::AbstractArray` : solution fields of the surfaces in 2D
- `splinep::AbstractArray` : x coordinates samples of the spline parameters of the border nodes
- `splineq::AbstractArray` : y coordinates samples of the spline parameters of the border nodes
- `mdl::model` : model object
- `pos2D::AbstractArray` : solution fields of the mesh in 2D
"""
function simulate(x0, x1, y0, y1, z0, z1, ne, Young, ν, ndim, FunctionClass, nDof, β, CameraMatrix, endTime, tSteps, Control, cParam, cMat; writeData=false, 
                    filepath=nothing, SIDES::Bool=false, dcdλ::Matrix{Float64}=zeros(Float64,1,1), dcdμ::Matrix{Float64}=zeros(Float64,1,1))
    ...
    mdl = def_model("linear_elasticity", ne=ne, NodeList=NodeListCylinder, IEN=IEN, IEN_top=IEN_top, IEN_btm=IEN_btm, ndim=ndim, nDof=nDof, ID = ID,
                     FunctionClass=FunctionClass, Young=Float64(Young), ν=ν, cMat=cMat, dcMatdλ=dcdλ, dcMatdμ=dcdμ) # define the model
    ...
    gradList = AbstractArray[zeros(size(BorderPts2D,1),size(BorderPts2D,2),3)]                                                 # store the solution fields of the border nodes in 2D 
    ...
    if Control == "force"
        for t in time
            ...
            K, dKdλ, dKdμ = assemble_system(mdl,GRAD=true)             # assemble the stiffness matrix
            ...
            dMdμ = [C_T*dKdμ*C_uc C_T*dKdμ*q_tp; q_tp'*dKdμ*C_uc q_tp'*dKdμ*q_tp]  # assemble the system of equations
            ...
            drdμ = [-C_T*dKdμ*q_btm; -q_tp'*dKdμ*q_btm]
            ...
            dsoldμ = M\(drdμ-dMdμ*sol)  # solve the system of equations
            ...
            dqfdμ = dsoldμ[1:end-1]    # solve the system of equations
            ...
            dμfdμ = dsoldμ[end]        # solve the system of equations
            ...
            dqdμ = dqdμ + C_uc*dqfdμ + dμfdμ*q_tp;           # assemble the solution
            ...
            dqdμ_out = hcat(dqdμ[ID[1,:]], dqdμ[ID[2,:]], dqdμ[ID[3,:]])'
            dqdβ_out = hcat(dqdβ[ID[1,:]], dqdβ[ID[2,:]], dqdβ[ID[3,:]])'

            dqdθ_out = cat(dqdλ_out,dqdμ_out,dqdβ_out,dims=(3,3)) # concatenate the gradients in to a tensor
    ...
    elseif Control == "displacement"
        for t in time
            ...
            dqdθ_out = cat(dqdλ_out,dqdβ_out,dqdμ_out,dims=(3,3)) # concatenate the gradients in to a tensor
