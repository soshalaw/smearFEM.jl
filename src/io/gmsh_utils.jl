...
"""
    _get_elements_for_physical(name, dim, tag_to_index) -> Matrix or nothing
...
"""
    _get_nodes_from_physical(name, dim, tag_to_index) -> Vector{Int} or nothing
...
"""
    _get_nodes_from_physical_with_boundary(name, dim, tag_to_index) -> Vector{Int} or nothing
...
"""
    get_mesh_data(filePath) -> (NodeList, IEN, IEN_top, IEN_bottom, IEN_lateral,
                                nNodes, node_sets, ne)
...
# Arguments
- `filePath::String`: path to the `.msh` file generated by Gmsh.

# Returns
- `NodeList::Matrix{Float64}`: 3×nNodes coordinate matrix.
- `IEN::Matrix{Int64}`: nNodesPerElem×nElem volume connectivity (Q1 or Q2 remapped).
- `IEN_top::Matrix{Int64}`: connectivity for the top-face physical group.
- `IEN_bottom::Matrix{Int64}`: connectivity for the bottom-face physical group.
- `IEN_lateral`: raw lateral-face connectivity.
- `nNodes::Int`: total number of nodes.
- `node_sets::Vector`: `[nodes_lateral, nodes_bottom, nodes_top]` sorted index vectors.
