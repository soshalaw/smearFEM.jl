...
function run_window_predictions(model, scene, conditions, est_ηpList, est_βpList,
    ...
    for ti::Int in 1:(size(data_ranges_, 1))
        ...
        if ti > 1
            ...
            pred_μ_list, gradList, pred_simBorderPts, fields_pred, surface_pts_3D_pred, pos2D_pred, pos3D_pred, _, _, _ =
                predict_window(model, scene, conditions,
                               (est_ηpList[data_range_prev][1], est_βpList[data_range_prev][1]))
        ...
        elseif gt_viscosity_type == "bulk_viscosity"
            ...
            if mode == "single_window"
                ...
                for ti::Int in 1:length(windows)
                    ...
                    stats = estimate_window(model, scene, conditions, obs_border_pt_lst_t, θ;
                                            outliers=outlier_frames, method=opt_method, opt_kw...)
                    ...
                    if ti > 1
                        @info "Predicting dynamics in time window $(ti) from window $(ti-1)'s estimate..."
                        pred_μ_list, _, pred_simBorderPts, fields_pred, surface_pts_3D_pred, pos2D_pred, pos3D_pred, _, _, _ =
                            predict_window(pred_model, pred_scene, conditions,
                                           (est_ηpList[data_range_prev][1], est_βpList[data_range_prev][1]))
...
batches, then per-experiment replotting and the cross-replicate post-analysis.
Invoked automatically when this file is run as a script
(`julia pipeline.jl`), but not when it is `include`d from the REPL or
another script.

This is a **multi-hour** run: the three batches refit every replicate serially.
To regenerate figures without refitting, call `plot_results()` directly.
"""
function main()
    data_types = ["simulated", "synthetic", "physical"]
    # Serial by default: a full `:gn` fit is memory-heavy enough that a few parallel workers
    # exhaust a typical dev machine (see `run_parallel_tasks`'s `memory_per_task_mb`).
    optimize_sim(false)
    optimize_syn(false)
    optimize_real(false)
    plot_results(data_types=data_types)
end
