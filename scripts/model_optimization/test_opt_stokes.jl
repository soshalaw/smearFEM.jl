...
global fs = 50  # font size for plots
...
function optimize(exp_params::Dict)
    ...
    elseif gt_viscosity_type == "bulk_viscosity"
        ...
        if mode == "single_window"
            ...
            println("observation Window: $(size(obsBorderPts_t,1))")
...
function compare_stats(filepath,filepath_gt)
    ...
    if length(path_1) != 0
        ...
        plot_η = set_plot(fs, sz=(2000, 1800))
        plot_β = set_plot(fs, sz=(2000, 1800))
        plot_cost = set_plot(fs, sz=(2000, 1800))
        plot_error = set_plot(fs, sz=(2000, 1800))
    ...
    if length(path_2) != 0
        ...
        plot_β = set_plot(fs, sz=(2000, 1800))
        plot_cost = set_plot(fs, sz=(2000, 1800))
        plot_cost_log = set_plot(fs, sz=(2000, 1800))
        plot_error = set_plot(fs, sz=(2000, 1800))
        plot_h = set_plot(fs, sz=(2000, 1800))
        ...
        for sim_time_folder in sim_time_folders
            ...
            for noise_folder in noise_folders
                ...
                if viscosity_type == "constant"
                    ...
                    η_gt::Float64 = float(sim_params["η"][1])
                    β_gt::Float64 = float(sim_params["β"][1])
                    ...
                    if noise_level == 0.0
                        ...
                        if sim_time_exp != sim_time
                            ...
                            if sim_time_exp < sim_time
                                ...
                                #     sim_time, t_steps)
                                #     est_μ_list, gradList, symBorderPts, fields, pos3D, pos2D, splinex, spliney = simulate(est_model, est_scene, conditions)
                        ...
                        elseif length(obsBorderPts) != length(symBorderPts)
                            if ne_exp == 6
                                if length(obsBorderPts) > length(symBorderPts)
                                    sim_time_ = (length(obsBorderPts)-1)*t_steps
                                    est_model, est_scene = def_problem(r, h, ne_exp, η_gt, ndim, FunctionClass_u, nDof_u, FunctionClass_p, nDof_p, FunctionClass_x, β_gt, F, control, viscosity_type, 
                                    sim_time_, t_steps)
                                    est_μ_list, gradList, symBorderPts, fields, pos3D, pos2D, splinex, spliney = simulate(est_model, est_scene, conditions)
                        ...
                        contour_plt = set_plot(fs, sz=(2000, 1800))
                        Plots.plot!(contour_plt, [], label=false, dpi=400, lw=3, legend=:outerbottom, legend_column=2, bottom_margin = -30mm, aspect_ratio = :equal)
                        Plots.scatter!(contour_plt, symBorderPts[end][1,:], symBorderPts[end][2,:], label="Simulated contour", dpi=400, lw=3, ms=:10, mc=:royalblue, ma=:0.7, markerstrokewidth=0.2)
                        Plots.plot!(contour_plt, nSplinex[end], nSpliney[end], label="Ground truth contour", dpi=400, color=:red, lw=3)
                        Plots.yflip!(true)
                        ...
                        contour_plt_zoom = set_plot(fs, sz=(1800, 1800))
                        Plots.plot!(contour_plt_zoom, [], label=false, dpi=400, lw=3, legend=:outerbottom, legend_column=2, bottom_margin = -30mm, aspect_ratio = :equal)
                        Plots.scatter!(contour_plt_zoom, symBorderPts[end][1,:], symBorderPts[end][2,:], label="Simulated contour", dpi=400, lw=3, ms=:10, mc=:royalblue, ma=:0.7, markerstrokewidth=0.2)
                        Plots.plot!(contour_plt_zoom, nSplinex[end], nSpliney[end], label="Ground truth contour", dpi=400, color=:red, lw=3)
                        ...
                        h_plt = set_plot(fs, sz=(2000, 1800))
                        ...
                        Plots.plot!(h_plt, time[1:n_time], est_h[1:n_time], label="Estimated height", dpi=400, lw=3, legend=:outerbottom, legend_column=2, bottom_margin = -30mm)
                        Plots.plot!(h_plt, time[1:n_time], gt_h[1:n_time], label="Ground truth height", dpi=400, lw=3, legend=:outerbottom, legend_column=2, bottom_margin = -30mm)
                        ...
                        Plots.savefig(joinpath(exp_path,"Results","plots","h_est_error.pdf"))
                        ...
                        Plots.savefig(joinpath(exp_path,"Results","plots","η.pdf"))
                        ...
                        Plots.savefig(joinpath(exp_path,"Results","plots","β.pdf"))
                        ...
                        Plots.savefig(joinpath(exp_path,"Results","plots","cost_steps.pdf"))
                        ...
                        Plots.xlabel!(L"\mathrm{Iterations}")
                        Plots.ylabel!(L"\mathrm{Cost\;(px)}")
                        Plots.savefig(joinpath(exp_path,"Results","plots","cost_steps_log.pdf"))
                        ...
                        try 
                            ...
                            try
                                ...
                                plt_dirs = set_plot(fs, sz=(2000, 1800))
                                ...
                                Plots.plot!(plt_dirs, est_η, est_β, label="Estimations", ms=:4, m=:x, color=:red, lw=3, legend=:outerbottom, legend_column=3, bottom_margin = -30mm, markerstrokewidth=0.1)
                                ...
                                plt_cont = set_plot(fs, sz=(2000, 1800))
                                ...
                                Plots.plot!(plt_cont, est_η, est_β, label="Estimations", ms=:4, m=:x, color=:red, lw=3, legend=:outerbottom, legend_column=3, bottom_margin = -30mm, markerstrokewidth=0.1)
                                ...
                                plt_slices = set_plot(fs, sz=(2000, 1800))
                        ...
                        # try
                            ...
                            if sum(mask_zoom) > 0
                                ...
                                plt_combined = set_subplot(fs, sz=(3500, 1800), layout=plt_layout)
                ...
                elseif viscosity_type == "bulk_viscosity"
                    ...
                    for window_dir in window_dirs
                        ...
                        # try 
                            ...
                            local β_gt::Vector{Float64} = float.(sim_params["β"])
                            ...
                            Plots.plot!(plt_cnt_error, t_full_h, d, label="Closest point distance error", dpi=400, lw=3, legend=:outerbottom, legend_column=2, bottom_margin = -30mm)
                            Plots.xlabel!(L"\mathrm{Time\;(s)}")
                            ...
                            for ti::Int in 1:size(data_ranges_, 1)
                                t = t_windows[ti]
                                Plots.vline!(plot_ratios, [t], color=:gray, lw=3, linestyle=:dash, label=false)
                            end
                            for ti::Int in 1:size(data_ranges_, 1)
                                t = t_windows[ti]
                                data_range_ = data_ranges_[ti]
                                println("Plotting ratio for data range: $data_range_")
                                t_win = collect(range(start=t_prev, stop=t, step=t_steps))
                                if ti == 1
                                    Plots.plot!(plot_ratios, t_win, ratio_opt[data_range_], label="Estimated η(t)", lw=3, color=:orange)
                                else
                                    Plots.plot!(plot_ratios, t_win, ratio_opt[data_range_], lw=3, color=:orange, label=false)
                                end
                                t_prev = t+t_steps
                            end
                            Plots.xlabel!(L"\mathrm{Time\;(s)}")
                            ...
                            Plots.plot(plt_η, [], label=false, dpi=400, lw=3, legend=:outerbottom, legend_column=2, bottom_margin = -30mm)
                            Plots.xlabel!(L"\mathrm{Time\;(s)}")
                            ...
                            if data_type != "physical"
                                ...
                                for ti::Int in 1:size(data_ranges_, 1)
                                    t = t_windows[ti]
                                    Plots.vline!(h_plt, [t], color=:gray, lw=3, linestyle=:dash, label=false)
                                end
                                Plots.plot!(h_plt, t_full_h, gt_h, label="Ground truth height", dpi=400, lw=3)
                                Plots.plot!(h_plt, t_full_h, est_h_list, label="Estimated height")
                                Plots.xlabel!(h_plt, L"\mathrm{Time\;(s)}")
                                Plots.ylabel!(h_plt, L"\mathrm{Height\;(mm)}")
                                Plots.savefig(joinpath(win_exp_path,"Results","plots","h.pdf"))

                                error_plt = set_plot(fs, sz=(2000, 1800))
                                for ti::Int in 1:size(data_ranges_, 1)
                                    t = t_windows[ti]
                                    Plots.vline!(error_plt, [t], color=:gray, lw=3, linestyle=:dash, label=false)
                                end
                                Plots.plot!(error_plt, t_full_h, abs.(est_h_list-gt_h), label="Height estimation error", dpi=400, lw=3)
                                ...
                                for ti::Int in 1:size(data_ranges_, 1)
                                    t = t_windows[ti]
                                    Plots.vline!(rel_error_plt, [t], color=:gray, lw=3, linestyle=:dash, label=false)
                                end
                                Plots.plot!(rel_error_plt, t_full_h, abs.(est_h_list-gt_h)./gt_h, label="Relative Height estimation error", dpi=400, lw=3)
                                Plots.xlabel!(rel_error_plt, L"\mathrm{Time\;(s)}")
                                Plots.ylabel!(rel_error_plt, L"\mathrm{Relative\;Height\;Error}")
...
function post_analysis_const(filepath_gt_::String, filepath::String, avoid_list)
    ...
    plot_conv_20 = set_plot(fs, sz=(2000, 1800))
    ...
    plot_conv_30 = set_plot(fs, sz=(2000, 1800))
    ...
    plot_conv_log_20 = set_plot(fs, sz=(2000, 1800))
    ...
    plot_conv_log_30 = set_plot(fs, sz=(2000, 1800))
    ...
    β_norm_plot_10 = set_plot(fs, sz=(2000, 1800))
    ...
    η_norm_plot_20 = set_plot(fs, sz=(2000, 1800))
    ...
    β_norm_plot_20 = set_plot(fs, sz=(2000, 1800))
    ...
    η_norm_plot_30 = set_plot(fs, sz=(2000, 1800))
    ...
    β_norm_plot_30 = set_plot(fs, sz=(2000, 1800))
    ...
    h_norm_plot_2 = set_plot(fs, sz=(2000, 1800))
    Plots.hline!(h_norm_plot_2, [1.0],  left_margin=12mm, bottom_margin = -30mm, linestyle=:dash, lw=3, label=false)
    Plots.xlabel!(h_norm_plot_2,L"\mathrm{Iterations}")
    Plots.ylabel!(h_norm_plot_2,L"\frac{h_{est}}{h_{gt}}")

    h_norm_plot_5 = set_plot(fs, sz=(2000, 1800))
    Plots.hline!(h_norm_plot_5, [1.0],  left_margin=12mm, bottom_margin = -30mm, linestyle=:dash, lw=3, label=false)
    Plots.xlabel!(h_norm_plot_5,L"\mathrm{Iterations}")
    Plots.ylabel!(h_norm_plot_5,L"\frac{h_{est}}{h_{gt}}")  

    h_norm_plot_10 = set_plot(fs, sz=(2000, 1800))
    Plots.hline!(h_norm_plot_10, [1.0],  left_margin=12mm, bottom_margin = -30mm, linestyle=:dash, lw=3, label=false)
    Plots.xlabel!(h_norm_plot_10,L"\mathrm{Iterations}")
    Plots.ylabel!(h_norm_plot_10,L"\frac{h_{est}}{h_{gt}}") 

    h_norm_plot_20 = set_plot(fs, sz=(2000, 1800))
    Plots.hline!(h_norm_plot_20, [1.0],  left_margin=12mm, bottom_margin = -30mm, linestyle=:dash, lw=3, label=false)
    Plots.xlabel!(h_norm_plot_20,L"\mathrm{Iterations}")
    Plots.ylabel!(h_norm_plot_20,L"\frac{h_{est}}{h_{gt}}") 

    h_norm_plot_30 = set_plot(fs, sz=(2000, 1800))
    Plots.hline!(h_norm_plot_30, [1.0],  left_margin=12mm, bottom_margin = -30mm, linestyle=:dash, lw=3, label=false)
    Plots.xlabel!(h_norm_plot_30,L"\mathrm{Iterations}")
    Plots.ylabel!(h_norm_plot_30,L"\frac{h_{est}}{h_{gt}}")
    ...
    Plots.ylabel!(ratio_plot_10,L"\frac{\eta_{est}}{\eta_{gt}}")

    ratio_plot_20 = set_plot(fs, sz=(2000, 1800))
    Plots.hline!(ratio_plot_20, [1.0],  left_margin=12mm, bottom_margin = -30mm, linestyle=:dash, lw=3, label=false)
    Plots.xlabel!(ratio_plot_20, L"\mathrm{Iterations}")
    Plots.ylabel!(ratio_plot_20, L"\frac{\beta_{est}}{\beta_{gt}}")

    ratio_plot_30 = set_plot(fs, sz=(2000, 1800))
    Plots.hline!(ratio_plot_30, [1.0],  left_margin=12mm, bottom_margin = -30mm, linestyle=:dash, lw=3, label=false)
    ...
    rel_height_error_glob_plot_20 = set_plot(fs, sz=(2000, 1800))
    Plots.xlabel!(rel_height_error_glob_plot_20, L"\mathrm{Time\;(s)}")
    Plots.ylabel!(rel_height_error_glob_plot_20, L"\mathrm{Relative\;Height\;Error\;(\%)}")

    rel_height_error_glob_plot_30 = set_plot(fs, sz=(2000, 1800))
    ...
    ratio_norm_plot_20 = set_plot(fs, sz=(2000, 1800))
    Plots.hline!(ratio_norm_plot_20, [1.0],  left_margin=12mm, bottom_margin = -30mm, linestyle=:dash, lw=3, label=false)
    ...
    ratio_norm_plot_30 = set_plot(fs, sz=(2000, 1800))
    Plots.hline!(ratio_norm_plot_30, [1.0],  left_margin=12mm, bottom_margin = -30mm, linestyle=:dash, lw=3, label=false)
    ...
    for dir in dir_list
        ...
        elem_η_plt = set_plot(fs, sz=(2000, 1800))
        Plots.hline!(elem_η_plt, [η_gt[1]], label="Ground truth η", lw=3,  left_margin=15mm, bottom_margin = -30mm)
        Plots.xlabel!(elem_η_plt,L"\mathrm{Iterations}")
        Plots.ylabel!(elem_η_plt,L"\eta\;\mathrm{(KPa\cdot s)}")

        elem_β_plt = set_plot(fs, sz=(2000, 1800))
        Plots.hline!(elem_β_plt, [β_gt[1]], label="Ground truth β", lw=3,  left_margin=15mm, bottom_margin = -30mm)
        Plots.xlabel!(elem_β_plt, L"\mathrm{Iterations}")
        Plots.ylabel!(elem_β_plt, L"\beta\;\mathrm{(mm^{-1})}")

        elem_ratio_plt = set_plot(fs, sz=(2000, 1800))
        Plots.hline!(elem_ratio_plt, [η_gt[1]/β_gt[1]], label="Ground truth η/β", lw=3,  left_margin=15mm, bottom_margin = -30mm)
        Plots.xlabel!(elem_ratio_plt, L"\mathrm{Iterations}")   
        Plots.ylabel!(elem_ratio_plt, L"\frac{\eta}{\beta}\;(KPa\cdot s \cdot mm^{-1})")
        
        # plots for normalized values
        elem_η_norm_plt = set_plot(fs, sz=(2000, 1800))
        Plots.hline!(elem_η_norm_plt, [1.0], label=false, lw=3,  left_margin=15mm, bottom_margin = -30mm, linestyle=:dash)
        Plots.xlabel!(elem_η_norm_plt,L"\mathrm{Iterations}")
        Plots.ylabel!(elem_η_norm_plt,L"\frac{\eta_{est}}{\eta_{gt}}")

        elem_β_norm_plt = set_plot(fs, sz=(2000, 1800))
        Plots.hline!(elem_β_norm_plt, [1.0], label=false, lw=3,  left_margin=15mm, bottom_margin = -30mm, linestyle=:dash)
        Plots.xlabel!(elem_β_norm_plt, L"\mathrm{Iterations}")
        Plots.ylabel!(elem_β_norm_plt, L"\frac{\beta_{est}}{\beta_{gt}}")

        elem_ratio_norm_plt = set_plot(fs, sz=(2000, 1800))
        Plots.hline!(elem_ratio_norm_plt, [1.0], label=false, lw=3,  left_margin=15mm, bottom_margin = -30mm, linestyle=:dash)
        ...
        elem_rel_height_error_plt = set_plot(fs, sz=(2000, 1800))
        Plots.plot!(elem_rel_height_error_plt, [], left_margin=15mm, bottom_margin = -30mm, label=false)
        ...
        elem_height_plt = set_plot(fs, sz=(2000, 1800))
        Plots.plot!(elem_height_plt, [], left_margin=15mm, bottom_margin = -30mm, label=false)
        ...
        for elem_size_folder_ in elem_size_folders
            ...
            sim_window_η_plt = set_plot(fs, sz=(2000, 1800))
            ...
            sim_window_β_plt = set_plot(fs, sz=(2000, 1800))
            ...
            sim_window_ratio_plt = set_plot(fs, sz=(2000, 1800))
            Plots.hline!(sim_window_ratio_plt, [η_gt[1]/β_gt[1]], label="Ground truth η/β", lw=3,  left_margin=15mm, bottom_margin = -30mm)
            Plots.xlabel!(sim_window_ratio_plt, L"\mathrm{Iterations}")   
            Plots.ylabel!(sim_window_ratio_plt, L"\frac{\eta}{\beta}\;(KPa\cdot s \cdot mm^{-1})")

            # plots for normalized values
            sim_window_η_norm_plt = set_plot(fs, sz=(2000, 1800))
            Plots.hline!(sim_window_η_norm_plt, [1.0], label=false, lw=3,  left_margin=15mm, bottom_margin = -30mm, linestyle=:dash)
            Plots.xlabel!(sim_window_η_norm_plt,L"\mathrm{Iterations}")
            Plots.ylabel!(sim_window_η_norm_plt,L"\frac{\eta_{est}}{\eta_{gt}}")

            sim_window_β_norm_plt = set_plot(fs, sz=(2000, 1800))
            Plots.hline!(sim_window_β_norm_plt, [1.0], label=false, lw=3,  left_margin=15mm, bottom_margin = -30mm, linestyle=:dash)
            Plots.xlabel!(sim_window_β_norm_plt, L"\mathrm{Iterations}")
            Plots.ylabel!(sim_window_β_norm_plt, L"\frac{\beta_{est}}{\beta_{gt}}")

            sim_window_ratio_norm_plt = set_plot(fs, sz=(2000, 1800))
            Plots.hline!(sim_window_ratio_norm_plt, [1.0], label=false, lw=3,  left_margin=15mm, bottom_margin = -30mm, linestyle=:dash)
            ...
            # plots for height error
            sim_window_rel_height_error_plt = set_plot(fs, sz=(2000, 1800))
            Plots.plot!(sim_window_rel_height_error_plt, [], left_margin=15mm, bottom_margin = -30mm, label=false)
            ...
            sim_window_height_plt = set_plot(fs, sz=(2000, 1800))
            Plots.plot!(sim_window_height_plt, [], left_margin=15mm, bottom_margin = -30mm, label=false)
            ...
            for sim_time_folder_ in reverse(sort(sim_time_folders))
                ...
                height_error_plt = set_plot(fs, sz=(2000, 1800))
                ...
                noise_fs::Int = round(Int, fs*1.34)
                noise_cols::Int = 2
                noise_plt_height::Int = 2300
                noise_lft_margin = 10mm

                covarience_plt = set_plot(noise_fs, sz=(2000, noise_plt_height))
                Plots.plot!(covarience_plt, [], left_margin=noise_lft_margin, label=false)
                Plots.xlabel!(covarience_plt, L"\eta\;\mathrm{(KPa\cdot s)}")
                Plots.ylabel!(covarience_plt, L"\beta\;\mathrm{(mm^{-1})}")

                height_noise_plt = set_plot(noise_fs, sz=(2000, noise_plt_height))
                Plots.plot!(height_noise_plt, [], left_margin=noise_lft_margin, label=false)
                Plots.xlabel!(height_noise_plt, L"\mathrm{Time\;(s)}")
                Plots.ylabel!(height_noise_plt, L"\mathrm{Height\;(mm)}")

                height_error_noise_plt = set_plot(noise_fs, sz=(2000, noise_plt_height))
                Plots.plot!(height_error_noise_plt, [], left_margin=noise_lft_margin, label=false)
                Plots.xlabel!(height_error_noise_plt, L"\mathrm{Time\;(s)}")
                Plots.ylabel!(height_error_noise_plt, L"\mathrm{Height\;Error\;(mm)}")

                rel_height_error_noise_plt = set_plot(noise_fs, sz=(2000, noise_plt_height))
                Plots.plot!(rel_height_error_noise_plt, [], left_margin=noise_lft_margin, label=false)
                ...
                for noise_folder_ in reverse(sort(noise_folders))
                    ...
                    gt_h = readdlm(joinpath(exp_path_n0,"Results","data","gt_h.csv"), ',', Float64)
                    ...
                    if noise_level == 0.0
                        ...
                        if size(est_h, 1) < (exp_points+1)
                            @warn "Estimated height data has fewer points ($(size(est_h, 1))) than expected ($(exp_points+1))..."
                            exp_points = size(est_h, 1)  - 1
                        end
                        ...
                        Plots.plot!(sim_window_η_plt, est_η, label=string("Window - $(sim_time)s"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=2)
                        ...
                        Plots.plot!(sim_window_ratio_norm_plt, normalized_ratio, label=string("Window - $(sim_time)s"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=2)
                        ...
                        try
                            ...
                            # 2D slices: cost vs distance along the two directions
                            if length(t_steep) > 0 && length(zs_steep) == length(t_steep)
                                Plots.plot!(plt_slices, t_steep, zs_steep, lw=3, legend = :outerright, label = "Steepest direction; Window = $(sim_time)s")
                            else
                                @warn "Skipping steep slice plot: empty or mismatched lengths: $(length(t_steep)) vs $(length(zs_steep))"
                            end

                            if length(t_flat) > 0 && length(zs_flat) == length(t_flat)
                                Plots.plot!(plt_slices, t_flat,  zs_flat, lw=3, legend = :outerright, label = "Flattest direction; Window = $(sim_time)s", legendfontsize=20)
                            else
                                @warn "Skipping flat slice plot: empty or mismatched lengths: $(length(t_flat)) vs $(length(zs_flat))"
                            end
                        catch e
                        ...
                        if sim_time == 2.0
                            if ne == 6
                                Plots.plot!(η_norm_plot_2, est_η_norm, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3)
                                Plots.plot!(β_norm_plot_2, est_β_norm, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3)
                                
                                Plots.plot!(plot_conv_2, cost_list, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3)
                                Plots.plot!(plot_conv_log_2, cost_list, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3, xscale=:log10, yscale=:log10)
                                
                                Plots.plot!(ratio_plot_2, ratio_est, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3, yscale=:log10)
                                Plots.hline!(ratio_plot_2, [ratio_gt], linestyle=:dash, lw=3, label=false, color=:black, yscale=:log10)

                                Plots.plot!(rel_height_error_glob_plot_2, time_h, rel_height_error, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), dpi=400, lw=3, legend=:outerbottom, legend_column=3, bottom_margin = -30mm)
                                Plots.plot!(ratio_norm_plot_2, normalized_ratio, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3)

                                Plots.plot!(h_norm_plot_2, h_norm, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), dpi=400, lw=3, legend=:outerbottom, legend_column=3)
                            end
                        elseif sim_time == 5.0
                            if ne == 6
                                Plots.plot!(η_norm_plot_5, est_η_norm, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3)
                                Plots.plot!(β_norm_plot_5, est_β_norm, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3)
                                
                                Plots.plot!(plot_conv_5, cost_list, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3)
                                Plots.plot!(plot_conv_log_5, cost_list, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3, xscale=:log10, yscale=:log10)
                                
                                Plots.plot!(ratio_plot_5, ratio_est, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3, yscale=:log10)
                                Plots.hline!(ratio_plot_5, [ratio_gt], linestyle=:dash, lw=3, label=false, color=:black, yscale=:log10)

                                Plots.plot!(rel_height_error_glob_plot_5, time_h, rel_height_error, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), dpi=400, lw=3, legend=:outerbottom, legend_column=3, bottom_margin = -30mm)
                                Plots.plot!(ratio_norm_plot_5, normalized_ratio, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3)

                                Plots.plot!(h_norm_plot_5, h_norm, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), dpi=400, lw=3, legend=:outerbottom, legend_column=3)
                            end
                        elseif sim_time == 10.0
                            if ne == 6
                                Plots.plot!(η_norm_plot_10, est_η_norm, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3)
                                Plots.plot!(β_norm_plot_10, est_β_norm, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3)
                                
                                Plots.plot!(plot_conv_10, cost_list, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3)
                                Plots.plot!(plot_conv_log_10, cost_list, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3, xscale=:log10, yscale=:log10)
                                
                                Plots.plot!(ratio_plot_10, ratio_est, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3, yscale=:log10)
                                Plots.hline!(ratio_plot_10, [ratio_gt], linestyle=:dash, lw=3, label=false, color=:black, yscale=:log10)

                                Plots.plot!(rel_height_error_glob_plot_10, time_h, rel_height_error, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), dpi=400, lw=3, legend=:outerbottom, legend_column=3, bottom_margin = -30mm)
                                Plots.plot!(ratio_norm_plot_10, normalized_ratio, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3)

                                Plots.plot!(h_norm_plot_10, h_norm, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), dpi=400, lw=3, legend=:outerbottom, legend_column=3)
                            end
                        elseif sim_time == 20.0
                            if ne == 6
                                Plots.plot!(η_norm_plot_20, est_η_norm, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3)
                                Plots.plot!(β_norm_plot_20, est_β_norm, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3)
                                
                                Plots.plot!(plot_conv_20, cost_list, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3)
                                Plots.plot!(plot_conv_log_20, cost_list, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3, xscale=:log10, yscale=:log10)
                                
                                Plots.plot!(ratio_plot_20, ratio_est, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3, yscale=:log10)
                                ...
                                Plots.plot!(ratio_norm_plot_20, normalized_ratio, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3)

                                Plots.plot!(h_norm_plot_20, h_norm, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), dpi=400, lw=3, legend=:outerbottom, legend_column=3)
                            end
                        elseif sim_time == 30.0
                            if ne == 6
                                Plots.plot!(η_norm_plot_30, est_η_norm, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3)
                                Plots.plot!(β_norm_plot_30, est_β_norm, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3)
                                
                                Plots.plot!(plot_conv_30, cost_list, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3)
                                Plots.plot!(plot_conv_log_30, cost_list, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3, xscale=:log10, yscale=:log10)
                                
                                Plots.plot!(ratio_plot_30, ratio_est, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3, yscale=:log10)
                                Plots.hline!(ratio_plot_30, [ratio_gt], linestyle=:dash, lw=3, label=false, color=:black, yscale=:log10)

                                Plots.plot!(rel_height_error_glob_plot_30, time_h, rel_height_error, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), dpi=400, lw=3, legend=:outerbottom, legend_column=2, bottom_margin = -30mm)
                                Plots.plot!(ratio_norm_plot_30, normalized_ratio, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3)

                                Plots.plot!(h_norm_plot_30, h_norm, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), dpi=400, lw=3, legend=:outerbottom, legend_column=3)
                            end
                        ...
                        plot_covariance!(covarience_plt, η_pred[:,1], β_pred[:,1], label=string(L"\sigma:\;",(round(noise_level,digits=2))," px  "), legend_column=noise_cols)
                        ...
                        StatsPlots.errorline!(height_noise_plt, time_h, h_pred', label=string(L"\sigma:\;",(round(noise_level,digits=2))," px  "), dpi=400, lw=3, legend=:outerbottom, legend_column=noise_cols, bottom_margin = -30mm)
                        StatsPlots.errorline!(height_error_noise_plt, time_h, height_error', label=string(L"\sigma:\;",(round(noise_level,digits=2))," px  "), dpi=400, lw=3, legend=:outerbottom, legend_column=noise_cols, bottom_margin = -30mm)
                        StatsPlots.errorline!(rel_height_error_noise_plt, time_h, rel_height_error', label=string(L"\sigma:\;",(round(noise_level,digits=2))," px  "), dpi=400, lw=3, legend=:outerbottom, legend_column=noise_cols, bottom_margin = -30mm)
                    end
                end
                # Plots.plot!(height_noise_plt, time, gt_h, label="Ground truth", dpi=400, lw=3, legend=:outerbottom, legend_column=noise_cols, bottom_margin = -30mm)
                Plots.scatter!(covarience_plt, η_gt, β_gt, label="Ground truth", ms=:15, m=:star5, color=:indianred2, markerstrokewidth=0.1, legend=:outerbottom, legend_column=noise_cols)
    ...
    for dir in dir_list
        ...
        for elem_size_folder_ in elem_size_folders
            ...
            for sim_time_folder_ in sim_time_folders
                ...
                win_folders = readdir(sim_time_folder)

                printstyled("Processing simulation time folder: $(sim_time_folder)\n", color=:cyan)
                for win_folder in win_folders

                    if win_folder == "post_analysis_windows" || win_folder == "Results" || win_folder == "single_window"
                        continue
                    end

                    exp_path = joinpath(filepath_dir, elem_size_folder_, sim_time_folder_, win_folder)
                    ...
                    elseif sim_time == 30.0
                        if ne == 6
                            ...
                            Plots.plot!(rel_height_error_glob_plot_30, time, rel_height_error, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), dpi=400, lw=3, legend=:outerbottom, legend_column=2, bottom_margin = -30mm)
                            Plots.plot!(ratio_norm_plot_30, normalized_ratio, label=string(L"\beta:\;",β_gt[1],L"\;\mathrm{mm}^{-1}"), marker=2, dpi=400, lw=3, legend=:outerbottom, legend_column=3)
                        end
...
function optimize_real()
    ...
    filepath_res::String = ""
    param_list = Vector{Dict}(undef, 0)
    sim_time_exp_list = [5.0, 10.0, 2.0] # simulation time in seconds
    ...
    for dir in dir_list
        ...
        for ne in refine_list
            ...
            for sim_time_exp in sim_time_exp_list
                ...
                for ne in refine_list
                    for FunctionClass_x in FunctionClass_x_List
                        filepath_res = string("/home/soshala/SMEAR-PhD/SMEAR-DataFiles/Data/experiments/physical_data/optimization/$dir/$(FunctionClass_x)_$(ne)/simtime_$(sim_time_exp)/$window")
                        @info "Running optimization with FunctionClass_x = $FunctionClass_x with $ne elements"
