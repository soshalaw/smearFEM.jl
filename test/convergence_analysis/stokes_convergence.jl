...
exp_ne = [2, 4, 8, 16]
error_iga_list = zeros(length(exp_ne))
error_fem_list = zeros(length(exp_ne))
error_dif_list = zeros(length(exp_ne))
...
for (i, ne) in enumerate(exp_ne)
    ...
    error_dif_list[i] = error_dif/length(iter)
end
...
set_plot(22)
Plots.plot!(exp_ne, error_dif_list, label="Difference", yscale=:log10)
