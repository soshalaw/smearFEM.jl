...
function run_exp(exp_params::Dict)
    ...
    obsBorderPts_fem, nSplinex, nSpliney, pd = add_noise(ObsDataList_fem, nFactor=noiseLevel)
    ...
    obsBorderPts_iga, nSplinex, nSpliney, pd = add_noise(ObsDataList_iga, nFactor=noiseLevel)
...
function main()
    ...
    ne_list = [2, 4, 8]
    η = 40
    error_list = zeros(length(β_gt_list),length(ne_list),21)
